...
export class AlexCreativeEngine extends EventEmitter  {
  ...
  optimizePromptForDALLE3(prompt, style) {
    ...
    const styleModifiers = {
      ...
      STR_CINEMATIC: ', cinematic lighting, dramatic composition, movie scene'
    };
  ...
  getActiveProviders() {
    ...
    Object.entries(this.imageProviders).forEach(([key, provider]) => {
      if (provider.enabled) {
        active.images.push({ key, name: provider.name });
      }
    });

    Object.entries(this.videoProviders).forEach(([key, provider]) => {
      if (provider.enabled) {
        active.videos.push({ key, name: provider.name });
      }
    });

    Object.entries(this.audioProviders).forEach(([key, provider]) => {
      if (provider.enabled) {
        active.audio.push({ key, name: provider.name });
      }
    });
