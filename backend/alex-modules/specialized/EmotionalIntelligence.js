...
export class EmotionalIntelligenceSystem extends EventEmitter  {
  ...
  generateValidation(state) {
    return await this.generateWithOpenAI(`Vos sentiments sont complètement compréhensibles.....`, context);
  }

  generateSupport(strategy) {
    return await this.generateWithOpenAI(`Je suis là pour vous accompagner dans cette démarc...`, context);
  }

  generateGuidance(context, state) {
    return await this.generateWithOpenAI(`Voici comment nous pouvons avancer ensemble......`, context);
  }
