...
class AlexSystemInitializer {
  ...
  async validateModules() {
    for (const modulePath of this.modules) {
      ...
      try {
        ...
        const hasSQLite =
          content.includes("sqlite") || content.includes("SQLite");
        const hasLearning =
          content.includes("learn") || content.includes("hybrid");

        const status = hasStaticLogic
          ? "⚠️"
          : hasSQLite && hasLearning
            ? "✅"
            : "🔄";
  ...
  async initializeSaaSArchitecture() {
    ...
    return new Promise((resolve, reject) => {
      db.run(
        `INSERT OR REPLACE INTO tenant_data 
